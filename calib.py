...
def cal_fit_poissongauss_panda(indexes, chi2, pss, covs):
    ...
    dpan = {'indexes': indexes,
            ...
            'entot': map(lambda cov: math.sqrt(abs(cov[0][0])), covs),
            'pedestal': map(lambda ps: ps[1], pss),
            'epedestal': map(lambda cov: math.sqrt(abs(cov[1][1])), covs),
            'gain': map(lambda ps: ps[2], pss),
            'egain': map(lambda cov: math.sqrt(abs(cov[2][2])), covs),
            'pes': map(lambda ps: ps[3], pss),
            'epes': map(lambda cov: math.sqrt(abs(cov[3][3])), covs),
            'noise': map(lambda ps: ps[4], pss),
            'enoise': map(lambda cov: math.sqrt(abs(cov[4][4])), covs),
            'noise-pe': map(lambda ps: ps[5], pss),
            'enoise-pe': map(lambda cov: math.sqrt(abs(cov[5][5])), covs)
            }
    pan = pd.DataFrame(dpan)
    return pan
